...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    contaningTuple = (dish_name,set(dish_ingredients))
    return contaningTuple
...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    if not (ALCOHOLS & (set(drink_ingredients))):
        bebida = drink_name + ' Mocktail'
    else:
        bebida = drink_name + ' Cocktail'
    return bebida 
...
def tag_special_ingredients(dish):
    ...
    plato = SPECIAL_INGREDIENTS & (set(dish[1]))
    platoRevisado = (dish[0], plato)
    return platoRevisado
...
def separate_appetizers(dishes, appetizers):
    ...
    """
    nuevoPlato = set(dishes).difference(set(appetizers))
    return list(nuevoPlato)
...
def singleton_ingredients(dishes, intersection):
    ...
    for items in dishes:
        ingredientes |= items
    return ingredientes.symmetric_difference(intersection)
